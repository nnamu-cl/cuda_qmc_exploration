...
Nsight Compute is not on this machine (`ncu` missing). Stall-reason quotes wait until it is installed. The timing already falsifies the DRAM-walk story.
...
Drop the 344 B DRAM model. The CDF walk is an L2 / divergence problem, and XORWOW state is 96 B of real traffic plus a 182 ms tax. Inverse-table work should kill the 23-step search, not chase `__expf`. Philox belongs in the endgame with the setup cost, not here.
