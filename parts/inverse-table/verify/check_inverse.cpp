...
bool CheckMoments(const GpuDraw& draw, qmc::cpu::QuantumNumbers qn,
  ...
  const bool ok = m_r.passed && m_r2.passed && m_inv.passed;
  fmt::print("  moments <r>={:.6f} <r2>={:.6f} <1/r>={:.6f} {}\n",
             m_r.sample_mean, m_r2.sample_mean, m_inv.sample_mean,
             gate_inv_r ? (ok ? "pass" : "FAIL") : "k4 interpolant");
  return !gate_inv_r || ok;
}
...
bool CheckAbVsNaive(qmc::cpu::QuantumNumbers qn, int n) {
  qmc::naive::GpuDraw naive;
  GpuDraw shared;
  qmc::naive::DrawGpu(qmc::naive::KernelKind::Fp32, qn, 0xC0FFEEULL, n, &naive);
  DrawGpu(KernelKind::SharedCdf, qn, 0xC0FFEEULL, n, &shared);
  int fail = 0;
  for (size_t i = 0; i < static_cast<size_t>(n); ++i) {
    if (shared.r[i] != naive.r[i] || shared.theta[i] != naive.theta[i] ||
        shared.phi[i] != naive.phi[i]) {
      ++fail;
    }
  }
  fmt::print("  A/B vs naive_fp32 fail={} {}\n", fail,
             fail == 0 ? "pass" : "FAIL");
  return fail == 0;
}
...
bool CheckKernel(KernelKind kind, int n, double max_d, bool full_orbitals) {
  ...
  for (int i = 0; i < n_orbitals; ++i) {
    ...
    const bool k4 = kind != KernelKind::SharedCdf;
    ok = CheckMoments(draw, qn, /*gate_inv_r=*/!k4) && ok;
    const bool noded = (qn.n - qn.l - 1) >= 1;
    if (kind == KernelKind::InverseTable) {
      fmt::print("  uniform quantile: bitwise A/B vs K2 is impossible\n");
      ok = CheckKs(draw, sampler, max_d) && ok;
      CheckChi2(draw, sampler, 128);
      if (noded && qn.n == 3 && qn.l == 1) {
        const auto node = NodeWindow(draw, sampler);
        fmt::print("  node-window chi2={:.1f} {}\n", node.chi2,
                   node.passed ? "pass" : "FAIL");
        if (n >= 10000000 && node.passed) {
          fmt::print("  expected (3,1) node trap did not fire\n");
          ok = false;
        }
      }
    } else if (kind == KernelKind::InverseNodes) {
      ok = CheckKs(draw, sampler, max_d) && ok;
      CheckChi2(draw, sampler, 128);
      if (noded && qn.n == 3 && qn.l == 1) {
        const auto node = NodeWindow(draw, sampler);
      ...
      ok = CheckKs(draw, sampler, max_d) && ok;
      ok = CheckChi2(draw, sampler, 128) && ok;
    }
...
int RunVerify(bool full) {
  ...
  fmt::print("shared_cdf A/B\n");
  const bool ab = CheckAbVsNaive({1, 0, 0}, full ? 4096 : 1024);
  const bool shared = CheckKernel(KernelKind::SharedCdf, n, max_d, full);
  const bool table = CheckKernel(KernelKind::InverseTable, n, max_d, false);
  const bool nodes = CheckKernel(KernelKind::InverseNodes, n, max_d, false);
  const bool ok = ab && shared && table && nodes;
  fmt::print("inverse-table verify-{} {}\n", full ? "full" : "fast",
             ok ? "pass" : "FAIL");
  return ok ? 0 : 1;
}
